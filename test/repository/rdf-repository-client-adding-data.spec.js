...
describe('RDFRepositoryClient - adding data', () => {
  ...
  describe('add(payload)', () => {
    test('should properly convert triple payload to quad and ' +
      'send a request', () => {
      ...
      const expected = testUtils
        .loadFile('repository/data/add-statements-triple.txt').trim();
      return rdfRepositoryClient.add(payload)
        .then(() => verifyAddPayload(expected));
    });

    test('should properly convert triple payload with blank nodes ' +
      'to quad and send a request', () => {
      const payload = new AddStatementPayload()
        .setSubject('_:1')
        .setPredicate(pred('relation-1'))
        .setObject('_:2');
      ...
      const expected = testUtils
        .loadFile('repository/data/add-statements-triple-bnodes.txt').trim();
      return rdfRepositoryClient.add(payload)
        .then(() => verifyAddPayload(expected));
    });

    test('should properly convert triple literal payload with language ' +
      'to quad and send a request', () => {
      const payload = new AddStatementPayload()
        .setSubject(subj('resource-1'))
        .setPredicate(pred('relation-1'))
      ...
      const expected = testUtils
        .loadFile('repository/data/add-statements-literal-language.txt').trim();
      return rdfRepositoryClient.add(payload)
        .then(() => verifyAddPayload(expected));
    });

    test('should properly convert triple literal payload with data type ' +
      'to quad and send a request', () => {
      const payload = new AddStatementPayload()
        .setSubject(subj('resource-1'))
      ...
      const expected = testUtils
        .loadFile('repository/data/add-statements-literal-boolean.txt').trim();
      return rdfRepositoryClient.add(payload)
        .then(() => verifyAddPayload(expected));
    });

    test('should properly convert string literal payload ' +
      'to quad and send a request', () => {
      const payload = new AddStatementPayload()
        .setSubject(subj('resource-1'))
      ...
      const expected = testUtils
        .loadFile('repository/data/add-statements-literal.txt').trim();
      return rdfRepositoryClient.add(payload)
        .then(() => verifyAddPayload(expected));
    });

    test('should properly convert string literal with language payload ' +
      'to quad and send a request', () => {
      const payload = new AddStatementPayload()
        .setSubject(subj('resource-1'))
        .setPredicate(pred('relation-1'))
    ...
    test('should properly convert integer literal payload ' +
      'to quad and send a request', () => {
      const payload = new AddStatementPayload()
        .setSubject(subj('resource-1'))
      ...
      const expected = testUtils
        .loadFile('repository/data/add-statements-literal-integer.txt').trim();
      return rdfRepositoryClient.add(payload)
        .then(() => verifyAddPayload(expected));
    });

    test('should properly convert float literal payload ' +
      'to quad and send a request', () => {
      const payload = new AddStatementPayload()
        .setSubject(subj('resource-1'))
      ...
      const expected = testUtils
        .loadFile('repository/data/add-statements-literal-decimal.txt').trim();
      return rdfRepositoryClient.add(payload)
        .then(() => verifyAddPayload(expected));
    });

    test('should properly convert boolean literal payload ' +
      'to quad and send a request', () => {
      const payload = new AddStatementPayload()
        .setSubject(subj('resource-1'))
        .setPredicate(pred('property-boolean'))
    ...
    test('should properly convert triple payload with single context ' +
      'to quad and send a request', () => {
      const payload = new AddStatementPayload()
        .setSubject(subj('resource-1'))
        .setPredicate(pred('relation-1'))
      ...
      return rdfRepositoryClient.add(payload)
        .then(() => verifyAddPayload(expected, expectedGraph));
    });

    test('should properly convert triple payload with multiple contexts ' +
      'to several quads and send a request', () => {
      ...
      const expectedGraphs =
        [encodedContext('data-graph-1'), encodedContext('data-graph-2')];
      const expected = testUtils
        .loadFile('repository/data/add-statements-contexts.txt').trim();
      return rdfRepositoryClient.add(payload)
        .then(() => verifyAddPayload(expected, expectedGraphs));
    });

    test('should allow to specify base URI for resolving ' +
      'of relative URIs', () => {
      const payload = new AddStatementPayload()
      ...
      const expected = testUtils
        .loadFile('repository/data/add-statements-triple.txt').trim();
      return rdfRepositoryClient.add(payload)
        .then(() => verifyAddPayload(expected, undefined, 'http://base/uri'));
    });
    ...
    test('should throw error when a payload is not provided', () => {
      expect(() => rdfRepositoryClient.add())
    ...
    test('should reject adding the payload when the server request ' +
      'is unsuccessful', () => {
  ...
  describe('addQuads(quads)', () => {
    ...
    test('should convert the quads to turtle and send a request', () => {
      ...
      const expected = testUtils
        .loadFile('repository/data/add-statements-complex.txt').trim();

      return rdfRepositoryClient.addQuads(quads)
        .then(() => verifyAddPayload(expected));
    });

    test('should allow to specify base URI and context ' +
      'when adding quads', () => {
      const quads = getQuadsDataSet();
    ...
    test('should reject adding quads when the server request ' +
      'is unsuccessful', () => {
      rdfRepositoryClient.httpClients[0].request
        .mockRejectedValue('error-adding');
      const quads = [getQuad('resource-1', 'relation-1', 'uri-1')];
      return expect(rdfRepositoryClient.addQuads(quads)).rejects
        .toEqual('error-adding');
    });
  });

  describe('putQuads(quads)', () => {
    test('should convert the quads to turtle and send an ' +
      'overwrite request', () => {
      const quads = getQuadsDataSet();
      ...
      return rdfRepositoryClient.putQuads(quads)
        .then(() => verifyPutPayload(expected));
    });

    test('should allow to specify base URI and context when ' +
      'putting quads', () => {
      const quads = getQuadsDataSet();
      const graph = context('data-graph-1');
      const baseUri = 'http://base/uri';
    ...
    test('should reject putting quads when the server request ' +
      'is unsuccessful', () => {
      rdfRepositoryClient.httpClients[0].request
        .mockRejectedValue('error-overwriting');
      const quads = [getQuad('resource-1', 'relation-1', 'uri-1')];
      return expect(rdfRepositoryClient.putQuads(quads)).rejects
        .toEqual('error-overwriting');
    });
  });
